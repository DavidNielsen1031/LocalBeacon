...
const LIMIT = limitArg ? parseInt(args[args.indexOf(limitArg) + 1] ?? limitArg.split('=')[1] ?? '999', 10) : 999
...
function generateReport(prospect: Prospect): string {
  ...
  const args: string[] = ['tsx', 'scripts/generate-report.ts', prospect.url, '--name', prospect.businessName]
  if (prospect.competitorURL && prospect.competitorName) {
    args.push('--competitor', prospect.competitorURL, '--competitor-name', prospect.competitorName)
  }
...
async function sendEmail(
  ...
  // Build multipart form for attachment, or JSON for no attachment
  let body: string
...
async function main() {
  ...
  // Filter to "Not Contacted" with a valid email
  const prospects = allProspects
    .filter(p => p.outreachStatus === 'Not Contacted' && p.contactEmail)
    .slice(0, LIMIT)

  console.log(`   ${prospects.length} eligible (Not Contacted + has email)\n`)
  ...
  for (let i = 0; i < prospects.length; i++) {
    ...
    console.log(`${idx} Processing: ${p.businessName} <${p.contactEmail}>`)

    try {
      // 1. Get or generate PDF
      const pdfPath = await ensureReport(p, DRY_RUN)
      ...
      if (DRY_RUN) {
        ...
        console.log(`  📊 Score: ${p.aeoScore}/100 vs ${p.competitorName}: ${p.competitorScore}/100`)
      ...
      // 5. Update status in Prospects tab
      markEmailed(p)
      console.log(`  🔄 Status updated to "Emailed" (row ${p.row})`)

      sent++

      // 6. Rate limit — wait before next send
      if (i < prospects.length - 1) {
        console.log(`  ⏳ Waiting ${RATE_LIMIT_MS / 1000}s before next send...`)
  ...
  if (DRY_RUN) {
    ...
    console.log(`   Would send: ${prospects.length}`)
