...
function DashboardPage() {
    ...
    var _b = (0, business_context_1.useBusinessContext)(), activeBusiness = _b.activeBusiness, activeBusinessId = _b.activeBusinessId, plan = _b.plan;
    var firstName = (_a = user === null || user === void 0 ? void 0 : user.firstName) !== null && _a !== void 0 ? _a : "there";
    var hasBusiness = !!activeBusiness;
    ...
    var stats = hasBusiness && data
        ? [
            ...
            { label: "AI Readiness", value: data.aeoScore !== null ? "".concat(data.aeoScore) : "—", sub: data.aeoScore !== null ? "out of 100" : "run your first scan", icon: "🤖", href: "/dashboard/ai-readiness" },
        ]
        : [
    ...
    var activity = hasBusiness && data && data.recentItems.length > 0
        ? data.recentItems.map(function (item) { return ({
            icon: item.type === "gbp_post" ? "📝" : item.type === "city_page" ? "🌐" : item.type === "review_reply" ? "⭐" : "📄",
            text: item.title || "".concat(item.type.replace("_", " "), " created"),
    ...
    return (<div className="flex-1 px-6 py-8 max-w-6xl">
      {/* Welcome banner */}
      <div className="mb-8">
        ...
        {hasBusiness && (data === null || data === void 0 ? void 0 : data.freshness) && (<div className="mt-3">
            <freshness_badge_1.FreshnessBadge daysSinceLastPost={data.freshness.daysSinceLastPost} status={data.freshness.status} lastPostDate={data.freshness.lastPostDate}/>
          </div>)}
      </div>

      {/* Setup CTA — only show if no business */}
      {!hasBusiness && (<card_1.Card className="bg-[#FFD700]/5 border-[#FFD700]/30 mb-8">
          <card_1.CardContent className="p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div>
            ...
            <link_1.default href="/onboarding">
              <button_1.Button className="bg-[#FFD700] text-black hover:bg-[#FFD700]/90 font-semibold whitespace-nowrap px-6">
                Connect Now →
              </button_1.Button>
            </link_1.default>
          </card_1.CardContent>
      ...
      {/* Loading state */}
      {loading && hasBusiness && (<div className="flex items-center justify-center py-12">
      ...
      {!loading && (<>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
            {stats.map(function (stat) { return (<link_1.default key={stat.label} href={stat.href}>
                <card_1.Card className="bg-white/5 border-white/10 hover:border-[#FFD700]/30 transition-colors cursor-pointer h-full">
          ...
          <div className="mb-10">
            ...
            <div className="space-y-3">
              {activity.map(function (item, i) { return (<div key={i} className="flex items-center gap-4 p-4 rounded-lg border transition-colors bg-white/5 border-white/10 hover:border-[#FFD700]/30 cursor-pointer">
                  <span className="text-xl">{item.icon}</span>
                  <div className="flex-1">
                    <p className="text-sm text-white">{item.text}</p>
                  </div>
                  <span className="text-xs text-white/30">{item.time}</span>
                  {item.action && (<link_1.default href={item.action}>
                      <button_1.Button size="sm" variant="ghost" className="text-[#FFD700] hover:text-[#FFD700]/80 text-xs">
                        →
                      </button_1.Button>
                    </link_1.default>)}
                </div>); })}
          ...
          {/* Download Report */}
          {hasBusiness && (<div className="mb-10">
              <card_1.Card className="bg-white/5 border-white/10">
                <card_1.CardContent className="p-5 flex flex-col sm:flex-row items-center justify-between gap-4">
                  <div>
                    <h3 className="text-white font-semibold text-sm mb-1">📄 Monthly Visibility Report</h3>
                    <p className="text-white/40 text-xs">
                      Download a PDF report for {activeBusiness === null || activeBusiness === void 0 ? void 0 : activeBusiness.name}. Share with clients or keep for your records.
                    </p>
                  </div>
                  <a href={"/api/reports/pdf?businessId=".concat(activeBusinessId)} download>
                    <button_1.Button size="sm" className="bg-[#FFD700] text-black hover:bg-[#FFD700]/90 font-semibold whitespace-nowrap px-6 text-xs">
                      Download Report
                    </button_1.Button>
                  </a>
                </card_1.CardContent>
          ...
          <div>
            ...
            ].map(function (action) { return (<card_1.Card key={action.title} className="bg-white/5 border-white/10 flex flex-col">
                  <card_1.CardContent className="p-5 flex-1 flex flex-col">
                    ...
                    <link_1.default href={action.href}>
                      <button_1.Button size="sm" className="w-full bg-[#FFD700] text-black hover:bg-[#FFD700]/90 font-semibold text-xs">
                        {action.cta}
                      </button_1.Button>
                    </link_1.default>
                  </card_1.CardContent>
