...
function generateLocalBusinessSchema(biz) {
    return {
        ...
        "areaServed": biz.areas.map(function (a) { return ({
            "@type": "City",
            "name": a
        }); }),
        "hasOfferCatalog": {
            "@type": "OfferCatalog",
            "name": "Services",
            "itemListElement": biz.services.map(function (s) { return ({
                "@type": "Offer",
                "itemOffered": {
                    "@type": "Service",
                    "name": s
...
function generateServiceSchema(biz) {
    return biz.services.map(function (service) { return ({
        "@context": "https://schema.org",
        "@type": "Service",
        "name": service,
        "provider": {
...
function SchemaPage() {
    ...
    var _b = (0, react_1.useState)('business'), activeTab = _b[0], setActiveTab = _b[1];
    var localBiz = generateLocalBusinessSchema(DEMO_BUSINESS);
    var faq = generateFAQSchema();
    var services = generateServiceSchema(DEMO_BUSINESS);
    var schemas = {
    ...
    var active = schemas[activeTab];
    var scriptTag = "<script type=\"application/ld+json\">\n".concat(JSON.stringify(active.data, null, 2), "\n</script>");
    var handleCopy = function (type) {
    ...
    return (<div className="p-6 md:p-8 max-w-5xl">
      ...
      <div className="flex gap-2 mb-6">
        {Object.keys(schemas).map(function (key) { return (<button key={key} onClick={function () { return setActiveTab(key); }} className={"px-4 py-2 rounded-lg text-sm font-medium transition-all ".concat(activeTab === key
      ...
      <div className="bg-[#111] border border-white/10 rounded-xl overflow-hidden">
        <div className="flex items-center justify-between p-4 border-b border-white/10">
          ...
          <button onClick={function () { return handleCopy(activeTab); }} className="bg-[#FFD700] text-black font-semibold px-4 py-2 rounded-lg text-sm hover:bg-[#FFD700]/90 transition-all">
