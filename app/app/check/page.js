...
var checker_form_1 = require("./checker-form");
exports.metadata = {
    ...
    return (<div className="min-h-screen bg-[#FAFAF7]">
      ...
      <nav className="border-b border-black/5 bg-white/80 backdrop-blur-sm">
        <div className="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <a href="/sign-up" className="text-sm font-medium text-[#1B2A4A] hover:text-[#FF6B35] transition-colors">
            Sign in →
          </a>
        </div>
      </nav>
      ...
      {/* Checker Form */}
      <react_1.Suspense fallback={<div className="max-w-2xl mx-auto px-6 pb-8"><div className="bg-white rounded-2xl shadow-lg border border-black/5 p-6 animate-pulse h-20"/></div>}>
        <checker_form_1.CheckerForm />
      </react_1.Suspense>
      ...
      <section className="max-w-3xl mx-auto px-6 py-16">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {[
            ...
            { step: '2', title: 'We scan 14 signals', desc: 'From AI discovery files to review content, we check what matters.' },
            { step: '3', title: 'Get your score', desc: 'See your grade instantly. Get the full breakdown via email.' },
        ].map(function (item) { return (<div key={item.step} className="text-center">
      ...
      <section className="bg-white border-y border-black/5">
        ...
        ].map(function (item) { return (<div key={item.label} className="flex items-center gap-3 p-3 rounded-lg bg-[#FAFAF7]">
